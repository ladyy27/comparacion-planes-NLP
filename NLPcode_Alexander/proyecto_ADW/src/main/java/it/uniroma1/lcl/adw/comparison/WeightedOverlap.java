...
/**
 * A non-parametric approach for comparing two multinomial distributions
 * proposed in:
 * 
 * M. T. Pilehvar, D. Jurgens and R. Navigli. Align, Disambiguate and Walk: A
 * Unified Approach for Measuring Semantic Similarity. ACL 2013.
 * 
 * The approach compares the ranking of the dimensions across the two
 * distributions. The approach penalizes the differences in the top-ranking
 * dimensions more than it does for the lower ones.
 * 
 * @author pilehvar
 *
 */
public class WeightedOverlap implements SignatureComparison {
	public double compare(SemSig v1, SemSig v2, boolean sortedNormalized) {
		...
		return compare(overlap, v1.getSortedIndices(), v2.getSortedIndices());
	}

	public double compare(TIntFloatMap v1, TIntFloatMap v2, boolean sorted) {
		TIntSet overlap = new TIntHashSet(v1.keySet());
		overlap.retainAll(v2.keySet());

		return compare(overlap, SemSigUtils.getSortedIndices(v1), SemSigUtils.getSortedIndices(v2));
	}

	/**
	 * New implementation of Weighted Overlap that better suits the case when
	 * only a small part of two vectors overlap, which is also the case for the
	 * provided vectors that are truncated to top-5000 elements (of the original
	 * 117,500) Also, to have a smoother distribution of scores, a square-root
	 * function has been added to the formula.
	 *
	 * @param v1
	 *            sorted list of dimensions in the first vector (smaller)
	 * @param v2
	 *            sorted list of dimensions in the second vector (larger)
	 * @return
	 */
	public static double compareSmallerWithBigger(TIntSet overlaps, int[] v1, int[] v2) {
		...
		while (iter.hasNext()) {
			...
			nominator += 1.0 / ((indexToPosition1.get(overlap) + 1) + (indexToPosition2.get(overlap) + 1));
			// nominator +=
			// 1.0/(Math.sqrt((map1.get(overlap)+1)+(map2.get(overlap)+1)));
